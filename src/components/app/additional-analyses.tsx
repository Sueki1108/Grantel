...
export function AdditionalAnalyses({ 
    ...
    if (activeTab === 'imobilizado') {
        ...
        const imobilizadoStatus = (processedData as any).imobilizadoStatus || {};
                            ...
                            ...getColumns(dataWithStatus.map(({ id, ...rest }) => rest)), // Hide 'id' column
                             {
                                ...
                                header: 'Ações',
                                cell: ({ row }) => {
                                    const originalItem = imobilizados.find(item => item.id === (row.original as any).id);
                                    if (!originalItem) return null;
                                    
                                    return (
                                        <div className="flex gap-2">
                                            <Button
                                                variant={(row.original as any).Classificação === 'Uso e Consumo' ? 'default' : 'outline'}
                                                size="sm"
                                            ...
                                            <Button
                                                variant={(row.original as any).Classificação === 'Ativo Imobilizado' ? 'default' : 'outline'}
                                                size="sm"
