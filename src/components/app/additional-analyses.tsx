...
function useReconciliation(siengeData: any[] | null, xmlItems: any[] | null) {
    return useMemo(() => {
        ...
        try {
            ...
            const h = {
                cnpj: findHeader(filteredSiengeData, ['cpf/cnpj', 'cpf/cnpj do fornecedor', 'cpf/cnpj do destinatario']),
            ...
            filteredSiengeData.forEach((row, index) => {
                const partnerCnpj = cleanAndToStr(row[h.cnpj!]);
                const key = `${cleanAndToStr(row[h.numero!])}-${partnerCnpj}`;
                if (!siengeMap.has(key)) siengeMap.set(key, []);
                siengeMap.get(key)!.push({...row, __siengeIndex: index });
            });

            const xmlMap = new Map<string, any[]>();
            xmlItems.forEach((row, index) => {
                const partnerCnpj = cleanAndToStr(row['CPF/CNPJ do Emitente']);
                const key = `${cleanAndToStr(row['Número da Nota'])}-${partnerCnpj}`;
                if (!xmlMap.has(key)) xmlMap.set(key, []);
                xmlMap.get(key)!.push({...row, __xmlIndex: index });
            });

            const getSiengeValueGetters = (siengeItem: any) => [
                () => parseFloat(String(siengeItem[h.valorTotal!] || '0').replace(',', '.')),
                ...(h.icmsOutras ? [() => parseFloat(String(siengeItem[h.icmsOutras!] || '0').replace(',', '.'))] : []),
                ...(h.desconto ? [() => parseFloat(String(siengeItem[h.valorTotal!] || '0').replace(',', '.')) + parseFloat(String(siengeItem[h.desconto!] || '0').replace(',', '.'))] : []),
                ...(h.frete ? [() => parseFloat(String(siengeItem[h.valorTotal!] || '0').replace(',', '.')) - parseFloat(String(siengeItem[h.frete!] || '0').replace(',', '.'))] : []),
                ...(h.ipiDespesas || h.icmsSt ? [() => parseFloat(String(siengeItem[h.valorTotal!] || '0').replace(',', '.')) - (h.ipiDespesas ? parseFloat(String(siengeItem[h.ipiDespesas] || '0').replace(',', '.')) : 0) - (h.icmsSt ? parseFloat(String(siengeItem[h.icmsSt] || '0').replace(',', '.')) : 0)] : []),
                ...(h.frete || h.ipiDespesas ? [() => parseFloat(String(siengeItem[h.valorTotal!] || '0').replace(',', '.')) - (h.frete ? parseFloat(String(siengeItem[h.frete] || '0').replace(',', '.')) : 0) - (h.ipiDespesas ? parseFloat(String(siengeItem[h.ipiDespesas] || '0').replace(',', '.')) : 0)] : []),
                ...(h.desconto || h.frete ? [() => parseFloat(String(siengeItem[h.valorTotal!] || '0').replace(',', '.')) + (h.desconto ? parseFloat(String(siengeItem[h.desconto] || '0').replace(',', '.')) : 0) - (h.frete ? parseFloat(String(siengeItem[h.frete] || '0').replace(',', '.')) : 0)] : []),
                ...(h.despesasAcessorias ? [() => parseFloat(String(siengeItem[h.valorTotal!] || '0').replace(',', '.')) - parseFloat(String(siengeItem[h.despesasAcessorias!] || '0').replace(',', '.'))] : []),
                ...(h.precoUnitario ? [() => parseFloat(String(siengeItem[h.precoUnitario!] || '0').replace(',', '.'))] : []),
            ];

             const getXmlValueGetters = (xmlItem: any) => [
                () => xmlItem['Valor Total'],
                () => xmlItem['Valor Total'], // Placeholder for icmsOutras
                () => xmlItem['Valor Total'], // Placeholder for discount
                () => xmlItem['Valor Total'], // Placeholder for freight
                () => xmlItem['Valor Total'], // Placeholder for IPI/ST
                () => xmlItem['Valor Total'], // Placeholder for freight/ipi
                () => xmlItem['Valor Total'], // Placeholder for discount/freight
                () => xmlItem['Valor Total'], // Placeholder for others
                () => xmlItem['Valor Unitário'], // For unit price match
            ];


            siengeMap.forEach((siengeItems, key) => {
                const xmlMatches = xmlMap.get(key);
                if (!xmlMatches) return;

                siengeItems.forEach(siengeItem => {
                    if (siengeItem.matched) return;
                    
                    const siengeValueGetters = getSiengeValueGetters(siengeItem);

                    for (let i = 0; i < xmlMatches.length; i++) {
                        const xmlItem = xmlMatches[i];
                        if (xmlItem.matched) continue;
                        
                        const xmlValueGetters = getXmlValueGetters(xmlItem);

                        for(let j=0; j < siengeValueGetters.length; j++) {
                             try {
                                const siengeValue = siengeValueGetters[j]();
                                const xmlValue = xmlValueGetters[j]();
                                
                                if (!isNaN(siengeValue) && !isNaN(xmlValue) && Math.abs(siengeValue - xmlValue) < 0.01) {
                                    reconciled.push({ ...xmlItem, ...Object.fromEntries(Object.entries(siengeItem).map(([k, v]) => [`Sienge_${k}`, v])), 'Observações': `Conciliado via ${j+1}ª chave` });
                                    xmlItem.matched = true;
                                    siengeItem.matched = true;
                                    return; // move to next sienge item
                                }
                            } catch {}
                        }
                    }
            ...
            const onlyInSienge = Array.from(siengeMap.values()).flat().filter(item => !item.matched);
            const onlyInXml = Array.from(xmlMap.values()).flat().filter(item => !item.matched);
            
            return { reconciliationResults: { reconciled, onlyInSienge, onlyInXml }, error: null };
        } catch (err: any) {
...
export function AdvancedAnalyses({ 
    ...
    const { reconciliationResults, error: reconciliationError } = useReconciliation(
        ...
        processedData.reconciliationResults?.allReconciledItems || []
    );
             ...
             <Card>
                <CardHeader>
                     <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                        ...
                        <Button onClick={onExportSession} size="sm" variant="outline"><Save className="mr-2 h-4 w-4"/>Guardar Sessão Atual</Button>
                    </div>
                </CardHeader>
             </Card>
                            ...
                            {reconciliationResults ? (
                                ...
                                siengeFile && (
                                    <div className="flex flex-col items-center justify-center min-h-[300px] text-muted-foreground border-2 border-dashed rounded-lg p-8">
                                        ...
                                        <p className="mt-4 text-center">Aguardando processamento inicial ou dados válidos para executar a conciliação...</p>
                                    </div>
                                )
                            )}
                        </CardContent>
